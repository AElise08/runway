...
import { CritiqueResult, AppState } from './types';
import { RefreshCw, Quote, Sparkles, X, ChevronDown, CameraOff, Clock } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const processImage = async (base64: string) => {
    ...
    setError(null);
    try {
      const jsonStr = await analyzeLook(base64);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-white selection:text-black transition-colors duration-1000">
      <Header />
            ...
            <div className="max-w-7xl mx-auto px-8 md:px-24 py-32 grid grid-cols-1 lg:grid-cols-12 gap-32">
              ...
              <div className="lg:col-span-4 space-y-20">
                <div className="p-12 bg-white/[0.02] border border-white/5 space-y-12 sticky top-40">
                  ...
                  <button 
                    onClick={reset}
                    className="w-full py-8 mt-10 bg-white text-black hover:bg-neutral-200 transition-all uppercase text-[11px] tracking-[0.7em] font-black group shadow-[0_0_40px_rgba(255,255,255,0.05)]"
                  >
                    <RefreshCw size={14} className="inline-block mr-4 group-hover:rotate-180 transition-transform duration-1000" /> Nova Audiência
                  </button>
                </div>
              </div>
            </div>
